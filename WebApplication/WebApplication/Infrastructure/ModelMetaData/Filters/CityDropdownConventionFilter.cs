...
{
    public class CityDropdownConventionFilter : IModelMetaDataFilter
    {
        public void TransformMetaData(ModelMetadata metadata, IEnumerable<Attribute> attributes)
        {
            if (!string.IsNullOrEmpty(metadata.PropertyName)
               && metadata.PropertyName.Contains("CityId")
               && string.IsNullOrEmpty(metadata.DataTypeName)
                )
            {
                metadata.DataTypeName = "CityId";
                //metadata.DisplayName = "City";
